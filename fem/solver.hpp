  ...
  SolverClass()
      : mpi_communicator(MPI_COMM_WORLD),
        ...
        triangulation(
          [this]() { 
            if constexpr (dim == 1) {
              // Costruttore per Triangulation standard (non richiede MPI)
              return dealii::Triangulation<dim>();
            } else {
              // Costruttore per Triangulation distribuita (richiede MPI)
              return dealii::parallel::distributed::Triangulation<dim>(
...
private:
  ...
  using TriangulationType = typename std::conditional<
      dim == 1,

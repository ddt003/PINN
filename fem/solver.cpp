      ...
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
        if (cell->face(f)->at_boundary()) {
          ...
          if (id != 1)
            ...
            cell->face(f)->set_boundary_id(boundary_id_inlet);
        }
  }
...
template class SolverClass<1, 2>;
template class SolverClass<2, 2>;
template class SolverClass<3, 2>;

template class SolverClass<1, 3>;
template class SolverClass<2, 3>;
template class SolverClass<3, 3>;

template class SolverClass<1, 5>;
template class SolverClass<2, 5>;

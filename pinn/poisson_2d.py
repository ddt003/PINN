...
2D Poisson equation with mixed boundary conditions
    ...
    Neumann:    du/dn = 0  on  x=0, x=1, y=1
    Dirichlet:  u = 0      on  y=0

Exact solution:  u(x,y) = x^2 (x-1)^2 y (y-1)^2

PINN setup:
  - Nf = 2000 collocation points in the domain, Ng = 250 on the boundary (LHS, resampled)
  - fully-connected network with tanh activation
  - Adam: 20000 iterations, lr = 1e-3, then L-BFGS
"""
import matplotlib
matplotlib.use("Agg")
...
def main():
    ...
    parser.add_argument("--eval-n", type=int, default=512,
                             help="Evaluation grid of eval-n x eval-n "
                             "(the paper uses grids up to 2000x2000)")
    ...
    for ax, U, title in zip(
            ...
            ["exact", "PINN", f"|error| (relative L2 = {err:.2e})"]):
